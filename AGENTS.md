...
- **No-Argument Stdin Handling**: Utilities that process files or stdin (e.g. `digest`, `jwt`) must never block indefinitely on `System.in` when executed interactively without arguments.
