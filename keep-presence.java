...
@Command(name = "keep-presence", mixinStandardHelpOptions = true, version = "keep-presence 1.2", description = "Simulates user activity (mouse movement, key press, scrolling) when idle to keep your presence status active.")
class keep_presence implements Callable<Integer> {
	...
	@Option(names = { "-s",
			"--seconds" }, description = "Define in seconds how long to wait between idle checks. Default 300.")
	private int seconds = 300;

	@Option(names = { "-b",
			"--buffer" }, description = "Initial buffer delay in seconds before the first check. Default: same as check interval.")
	...
	@Option(names = { "-r",
			"--random" }, arity = "2", paramLabel = "<START> <STOP>", description = "Execute actions using a random interval between START and STOP seconds. Overrides --seconds.")
	...
	public Integer call() {
		if (GraphicsEnvironment.isHeadless()) {
			System.err
				.println("Error: Headless environment detected. java.awt.Robot requires a desktop GUI environment.");
			return 1;
		}
	...
	private Point moveMouse(Point current) {
		...
		if (actual != null && actual.equals(current) && !circular) {
			log(String.format("Moved mouse out (%d, %d) and back to (%d, %d)", current.x + step, current.y + step,
					current.x, current.y));
		} else if (actual != null) {
