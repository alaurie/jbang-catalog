...
/// Reach is an advanced network diagnostic CLI tool to test TCP connectivity,
/// measure handshake latency, inspect TLS/SSL certificates, probe HTTP/HTTPS
/// status, and query DNS & WHOIS.
@Command(name = "reach", mixinStandardHelpOptions = true, version = "reach 2.0", description = "Network diagnostic CLI utility to test TCP reachability and inspect TLS certs.")
...
class Reach implements Callable<Integer> {
	...
	@Parameters(index = "1", arity = "0..1", description = "Port, list (80,443), or range (80-85). Default: 80 or 443.")
	...
	@Option(names = { "-n",
			"--count" }, defaultValue = "4", description = "Number of probe attempts per port (default: 4).")
	private int count = 4;

	@Option(names = { "-c", "--continuous" }, description = "Continuous probing until stopped via Ctrl+C.")
	private boolean continuous;

	@Option(names = {
			"--dns" }, description = "Perform comprehensive DNS records lookup (A, AAAA, MX, NS, CNAME, TXT).")
	private boolean checkDns;

	@Option(names = {
			"--whois" }, description = "Perform native WHOIS domain lookup (Registrar, Creation & Expiration dates).")
	private boolean checkWhois;

	@Option(names = { "-t",
			"--timeout" }, defaultValue = "2000", description = "Connection timeout in milliseconds (default: 2000).")
	private int timeout = 2000;

	@Option(names = { "-i",
			"--interval" }, defaultValue = "1000", description = "Interval between probes in milliseconds (default: 1000).")
	...
	@Option(names = { "-H",
			"--http" }, description = "Probe Layer 7 HTTP/HTTPS status code and Time-To-First-Byte (TTFB).")
	...
	@Option(names = { "-w",
			"--warn-days" }, description = "Exit code 2 if SSL certificate expires within specified days threshold.")
	...
	public Integer call() {
		...
		if (checkDns || checkWhois) {
			try (var executor = Executors.newVirtualThreadPerTaskExecutor()) {
				var dnsFuture = checkDns ? CompletableFuture.supplyAsync(() -> inspectDnsRecords(host), executor)
						: null;
				var whoisFuture = checkWhois ? CompletableFuture.supplyAsync(() -> queryWhois(host), executor) : null;
				if (dnsFuture != null)
		...
		for (var port : ports) {
			...
			if (!jsonOutput) {
				...
				if (checkHttp && httpInfo != null) {
					if (httpInfo.error() == null) {
						var serverStr = httpInfo.serverHeader() != null ? " [Server: " + httpInfo.serverHeader() + "]"
								: "";
			...
			var lossPercent = transmitted > 0 ? ((transmitted - received) / (double) transmitted) * 100.0 : 0.0;
			DoubleSummaryStatistics stats = rtts.stream().mapToDouble(Double::doubleValue).summaryStatistics();
		...
		for (var i = 0; i < results.size(); i++) {
			...
			if (res.tls() != null) {
				...
				if (res.tls().error() == null) {
					sb.append("        \"subject\": \"")
						.append(escapeJson(res.tls().subject()))
						.append("\",\n");
					sb.append("        \"issuer\": \"")
						.append(escapeJson(res.tls().issuer()))
						.append("\",\n");
					...
					sb.append("        \"protocol\": \"")
						.append(escapeJson(res.tls().protocol()))
						.append("\",\n");
					sb.append("        \"cipher\": \"")
						.append(escapeJson(res.tls().cipherSuite()))
						.append("\"\n");
				} else {
