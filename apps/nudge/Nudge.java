...
//FILES META-INF/native-image/nudge/reachability-metadata.json=META-INF/native-image/nudge/reachability-metadata.json
...
class Nudge implements Callable<Integer> {
	...
	@Option(names = { "-b",
			"--buffer" }, description = "Initial buffer delay in seconds before the first check. Default: same as check"
					+ " interval.")
	...
	@Option(names = { "-r",
			"--random" }, arity = "2", paramLabel = "<START> <STOP>", description = "Execute actions using a random interval between START and STOP seconds. Overrides"
					+ " --seconds.")
	private List<Integer> randomRange;

	@Option(names = {
			"--between" }, arity = "2", paramLabel = "<START> <STOP>", description = "Only perform nudges between HH:mm and HH:mm working hours window.")
				...
				try {
					...
					try {
						new ProcessBuilder("gdbus", "call", "--session", "--dest",
								"org.freedesktop.ScreenSaver", "--object-path", "/org/freedesktop/ScreenSaver",
								"--method", "org.freedesktop.ScreenSaver.UnInhibit", cookie)
							.start()
							.waitFor();
					} catch (Exception _) {
